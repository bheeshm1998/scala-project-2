...
object SparkStreamingJob {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val inputTopic = "server-metrics"
    val outputTopic = "protobuf-topic"
    ...
    val protobufMetrics = metrics.map { row =>
      ...
      val metric = Metric2.of(row.metricName, row.value, row.timestamp, row.host, row.region)
